...
struct Logday: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 0) {
                // Fixed header (Activity + icons) — stays static
                HStack {
                    Text("Activity")
    ...
    struct WeekView: View {
        ...
        var body: some View {
            VStack(spacing: 12) {
                // Header row inside card (unchanged)
                HStack {
                    Text(currentDate.formatted(.dateTime.month(.wide).year()))
                        .font(.title2).bold()
                    
                    Button {
                        // Toggle dropdown and preload current month/year
                        tempMonth = Calendar.current.component(.month, from: currentDate)
                        tempYear = Calendar.current.component(.year, from: currentDate)
                        withAnimation(.spring(response: 0.3, dampingFraction: 0.9)) {
                            isDropdownOpen.toggle()
                        }
                    } label: {
                        Image(systemName: isDropdownOpen ? "chevron.down" : "chevron.right")
                            .foregroundColor(Color.orangeBASIC)
                            .bold()
                ...
                // Dropdown content vs week row (unchanged behavior)
                if isDropdownOpen {
                    dropdownContent
        ...
        var body: some View {
            ScrollViewReader { proxy in
                ...
                .onAppear {
                    ...
                    let target = LogdayViewModel.startOfMonth(for: selectedDate)
        ...
        private struct MonthSection: View {
            ...
            var body: some View {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    HStack {
                        ForEach(LogdayViewModel.weekdaySymbolsShort(), id: \.self) { symbol in
                    ...
                    VStack(spacing: 12) {
                        ForEach(LogdayViewModel.monthGrid(for: monthStart), id: \.self) { week in
                            HStack(spacing: 0) {
